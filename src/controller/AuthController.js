...
//                 .json({ success: false, message: "Tài khoản không tồn tại !" });
//         }
//         let [valueOTP] = await pool.query(
//             `SELECT * FROM otp WHERE otp = ? ORDER BY createAt DESC`,
//             [otp]
//         );
//         if (valueOTP[0]?.email !== userExist[0].email) {
//             return res.status(400).json({
//                 success: true,
//                 message: "Mã OTP không hợp lệ với email này !",
//             });
//         }
...
//                 "UPDATE user set password = ?, updateAt=? WHERE email = ?",
//                 [hashedPassword, currentDateTime, valueOTP[0]?.email]
//             );
//             await pool.query(`DELETE FROM otp WHERE email = ?`, [valueOTP[0]?.email]);
//             return res
//                 .status(200)
//                 .json({ success: true, message: "Bạn đã đổi mật khẩu thành công" });
//         }

//         return res
//             .status(400)
//             .json({ success: true, message: "Mã OTP đã hết hạn !" });
//     } catch (error) {
...
//                 [valueOTP[0]?.email, hashedPassword, username]
//             );
//             await pool.query(`DELETE FROM otp WHERE email = ?`, [valueOTP[0]?.email]);
//             return res
//                 .status(200)
//                 .json({ success: true, message: "Bạn đã đăng ký thành công" });
//         }
...
//     try {
//         const [userExist] = await pool.query(`SELECT * FROM user WHERE email = ?`, [
//             email,
//         ]);
//         if (userExist.length > 0) {
//             return res
...
//                 success: true,
//                 message: "Người dùng mới đã được chèn vào cơ sở dữ liệu",
//             });
//         }
//     } catch (error) {
//         return res
